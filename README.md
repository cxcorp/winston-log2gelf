...
Additional options when using the `tcp` or `tls` protocol
...
Additional options when using the `http` or `https` protocol

* `path`: Pathname of the HTTP endpoint. Useful if Graylog's HTTP input is running behind a reverse proxy. (default: `/gelf`).
